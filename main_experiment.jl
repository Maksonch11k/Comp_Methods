...
# ===================================================================
#   ЧАСТЬ 2 (Без изменений)
# ===================================================================
println("\n\n--- Часть 2: Уравнение Фредгольма I рода ---")
println("\n--- Пример 2.1: Гауссово ядро (с поиском оптимального alpha) ---")
K_2_1(t, x) = exp(-(t - x)^2)
...
alphas_to_test_1 = [0.1, 0.05, 0.02, 0.01, 0.005, 0.002]
errors_1 = []
t_plot_2_1 = range(a_2_1, stop=b_2_1, length=200)
exact_vals_2_1 = u_exact_2_1.(t_plot_2_1)
for alpha in alphas_to_test_1
    u_approx_func = solve_fredholm_I_via_Wazwaz_regularization(K_2_1, g_noisy_2_1, a_2_1, b_2_1, n_2_1, alpha)
...
println("Найден оптимальный alpha = $best_alpha_1 с минимальной ошибкой = $min_error_1")
best_u_approx_1 = solve_fredholm_I_via_Wazwaz_regularization(K_2_1, g_noisy_2_1, a_2_1, b_2_1, n_2_1, best_alpha_1)
p2_1 = plot(t_plot_2_1, u_exact_2_1, label="Точное решение", lw=2)
plot!(p2_1, t_plot_2_1, best_u_approx_1, label="Решение (alpha=$best_alpha_1)", ls=:dash, lw=2)
title!("Часть 2, Пример 1")
savefig(p2_1, "fredholm_I_ex1.png")

println("\n--- Пример 2.2: Ядро cos(t*x) (с поиском оптимального alpha) ---")
K_2_2(t, x) = cos(t * x)
u_exact_2_2(t) = 1.0 + t
...
alphas_to_test_2 = [0.1, 0.05, 0.02, 0.01, 0.005, 0.002]
errors_2 = []
t_plot_2_2 = range(a_2_2, stop=b_2_2, length=200)
exact_vals_2_2 = u_exact_2_2.(t_plot_2_2)
for alpha in alphas_to_test_2
    u_approx_func = solve_fredholm_I_via_Wazwaz_regularization(K_2_2, g_noisy_2_2, a_2_2, b_2_2, n_2_2, alpha)
...
println("Найден оптимальный alpha = $best_alpha_2 с минимальной ошибкой = $min_error_2")
best_u_approx_2 = solve_fredholm_I_via_Wazwaz_regularization(K_2_2, g_noisy_2_2, a_2_2, b_2_2, n_2_2, best_alpha_2)
...
savefig(p2_2, "fredholm_I_ex2.png")
...
# --- Пример 3.1: Простой и устойчивый пример ---
println("\n--- Пример 3.1: Простой полиномиальный пример ---")
K_3_1(t, x) = t*x
f_3_1(t) = t - t^4/3 # Правильная f(t) для u(t)=t
a_3_1, b_3_1 = 0.0, 2.0 # Интервал можно взять побольше
u_exact_3_1(t) = t
n_3_1 = 40
...
# --- Пример 3.2: Исходный пример, но исправленный и на коротком интервале ---
println("\n--- Пример 3.2: Пример с sin(t) на коротком интервале ---")
K_3_2(t, x) = t - x
# ИСПРАВЛЕННАЯ правая часть
f_3_2(t) = 2*sin(t) - t
# СОКРАЩЕННЫЙ интервал для стабильности
a_3_2, b_3_2 = 0.0, π/2 
u_exact_3_2(t) = sin(t)
n_3_2 = 40
